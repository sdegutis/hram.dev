...
extern "C" int asm_exec(lua_State* L) {
	return 0;
...
extern "C" int luaopen_asm(lua_State* L) {
	luaL_newlib(L, asmlib);

	auto base = (UINT8*)0x33000;
	...
	if (err) {
		printf("ERROR 1: %08x (%s)\n", err, asmjit::DebugUtils::errorAsString(err));
		return 0;
	}

	err = code.flatten();
	if (err) {
		printf("ERROR 2: %08x (%s)\n", err, asmjit::DebugUtils::errorAsString(err));
		return 0;
	}
	...
	err = code.copyFlattenedData(base, 0x1000, asmjit::CopySectionFlags::kPadSectionBuffer);
	if (err) {
		printf("ERROR 3: %08x (%s)\n", err, asmjit::DebugUtils::errorAsString(err));
		return 0;
	}
